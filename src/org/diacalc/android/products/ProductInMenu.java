...
public class ProductInMenu extends ProductW{
   ...
   public ProductInMenu(String newName, float newProt, float newFat, float newCarb,
            int newGi, float newWeight, int idProd){
        super(newName, newProt, newFat, newCarb, newGi,newWeight);
        this.idProd = idProd;
    }
    public ProductInMenu(ProductInBase pr){
        super(pr.getName(),
                pr.getProt(), pr.getFat(), pr.getCarb(), pr.getGi(),0f);
        this.idProd = pr.getId();
    }
    ...
    public ProductInMenu(ProductInSearch pr) {
    	super(pr.getName(),
                pr.getProt(), pr.getFat(), pr.getCarb(), pr.getGi(),0f);
        this.idProd = pr.getId();
	}
